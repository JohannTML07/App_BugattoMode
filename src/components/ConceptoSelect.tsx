...
export function ConceptoSelect({ options }: { options: Option[] }) {
  ...
  return (
    <div className="relative w-full" ref={ref}>
      <input type="hidden" name="concepto_id" value={selected?.id || ''} required />
      ...
      >
        <span className={`truncate text-sm ${selected ? 'text-gray-900 font-medium' : 'text-gray-500'}`}>
          {selected ? selected.label : 'Buscar concepto...'}
        </span>
      ...
      {open && (
        <div className="absolute z-50 w-full mt-2 bg-white border border-gray-100 rounded-xl shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-100">
          ...
          <div className="max-h-56 overflow-y-auto py-1">
            {filtered.length === 0 ? (
              <div className="p-4 text-sm text-gray-500 text-center">No se encontraron resultados.</div>
            ) : (
              filtered.map(opt => (
                <div 
                  key={opt.id}
